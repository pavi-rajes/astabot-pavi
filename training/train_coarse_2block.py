"""
Train DynamicRoutingRNNCoarse with a SINGLE 2-block BPTT window (180 trials × 25 steps).

Key difference from train_coarse.py: instead of 2 separate 90-trial BPTT windows,
a single 180-trial window spans BOTH blocks. Gradient flows from Block 2 instruction
trials all the way back to Block 1 activity — network sees inter-block context contrast.

Usage:
    conda run -n latent_circuit python training/train_coarse_2block.py \
        --outdir checkpoints_coarse_2block --seed 42
"""
...
def main():
    ...
    while curriculum.stage <= 4 and total_steps < cfg.max_steps * 5:
        ...
        if stage == 3:
            ...
            stage_bptt   = 180  # full 2-block session (180 × 25 = 4500 steps per window)
            stage_bptt_w = 1    # single window spanning both blocks — gradient crosses boundary
            stage_max_sr = 1.0
        else:
            stage_bptt   = 15
            stage_bptt_w = 8
