"""
Train IntegratorRNNCoarse: leaky reward integrator + dt=100ms, tau=500ms.

Combines two approaches that individually failed:
- Integrator alone (γ=0.90): W_in[:,7]=0 at init → no gradient foothold
- Coarse dt alone: 60% context/trial, but decays to ~0 by mid-block

Combined fixes:
1. W_in[:,7] initialized with small nonzero weights (0.05) → gradient from step 1
2. dt=100ms → 5x faster training
3. Integrator c≈0.41 after 5 instruction trials → persistent context signal
4. gamma=0.90 → tau≈10 trials (reaches near saturation within first block)

Usage:
    conda run -n latent_circuit python training/train_integrator_coarse.py \
        --outdir checkpoints_integrator_coarse --gamma 0.90 --seed 42
"""
...
def get_config():
    ...
    p.add_argument('--outdir', default='checkpoints_integrator_coarse')
